...
import { DndContext } from "@dnd-kit/core"; // dnd-kit replacement
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.time('retroChannelJoin -> received "ok"');
  retroChannel.join()
    .receive("error", (error) => console.error(error))
    .receive("ok", (initialState) => {
      console.timeEnd('retroChannelJoin -> received "ok"');
            ...
            <Provider store={store}>
              <DndContext
                onDragEnd={(event) => {
                  ...
                  if (active && over) {
                    actionz.handleDragEnd(active, over);
                  }
                }}
              >
                <RemoteRetro />
